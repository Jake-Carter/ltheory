...
vec3 nebulaPaletteSample (
    vec3 dir, samplerCube map, vec3 starColor, float chromaVariance, float lod)
{
  vec3 baked = textureCubeLod(map, dir, lod).xyz;
  vec3 c = nebulaStarPalette(baked, starColor, chromaVariance);
  c += nebulaAmbientHaze(baked, starColor, 0.12);
  return c;
}

/* Emissive scatter — lit by star + skybox palette, adds light instead of blocking it. */
vec3 shadeDustCloudScatter (
    vec3 eye, vec3 pos, vec3 starDir, vec3 starColor,
    ...
    float scatterIntensity)
{
  ...
  /* Sky-anchored base — varies with view color but not sun-facing intensity. */
  vec3 scatter = nebulaPaletteSample(Vn, envMap, starColor, chromaVariance, 2.5)
    * nebulaGIIntensity * 1.4 * gas;
  scatter += linear(textureCubeLod(irMap, Vn, 4.0).xyz) * nebulaGIIntensity * 0.55 * gas;

  /* Subtle star tint; kept nearly isotropic to avoid sun-in-view blowout. */
    ...
    float scatterIntensity, float streakT)
{
  ...
  scatter += nebulaPaletteSample(Vn, envMap, starColor, chromaVariance, 3.0)
